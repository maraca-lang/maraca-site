[
  ...
  [
    ...
    "In Maraca, adjacent expressions are 'combined'. There are different cases
    depending on the types being combined (nil, value, or list).",
    "Multiple adjacent expressions next are combined pairwise, from left to
  ...
  [
    ...
    "Combining two nil/value expressions concatenates them together, treating
    nil as the empty string.",
    "If neither value is nil and there are any spaces between the expressions, a
    single space is added between the them.",
  ...
  [
    ...
    "Combining a nil/value expression with list performs a list key lookup. The
    order of the expressions, and any spaces between them, have no effect.",
  ...
  [
    ...
    "There is one case where the dot operator performs a different operation to
    a standard combination.",
    "If nil and a value are combined, rather than concatenating them, the dot
    operator treats the nil as an empty list and hence returns nil. This allows
    for nested key lookups on potentially nil values.",
